...
import { Holiday, HolidaysResponse } from "../../data/holiday/holiday.model";
...
export const getHolidaysAsync = createAsyncThunk(
  ...
  async (params: TableFilterState) => {
    const response: AxiosResponse<BaseResponse<HolidaysResponse>> = await GET(
      "/holidays",
      {
        params,
      }
...
export const addHolidayAsync = createAsyncThunk(
  ...
  async (request: Holiday) => {
    const response: AxiosResponse<BaseResponse<Holiday>> = await POST(
      "/holidays",
      request
    );
    return response.data.data;
